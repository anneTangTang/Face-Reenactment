...
def angle2matrix(angles: Sequence[float], is_3ddfa: bool = False) -> Any:
    """Get rotation matrix from three rotation angles, right-handed.

    :param angles: x, y, z angles, in radian if is_3ddfa else in degree
    :param is_3ddfa: True only when processing 300W_LP data
    :return: 3x3 rotation matrix
    """
    if is_3ddfa:
        x, y, z = angles[0], angles[1], angles[2]
        rotation_x = np.array([[1, 0, 0], [0, cos(x), sin(x)], [0, -sin(x), cos(x)]])
        rotation_y = np.array([[cos(y), 0, -sin(y)], [0, 1, 0], [sin(y), 0, cos(y)]])
        rotation_z = np.array([[cos(z), sin(z), 0], [-sin(z), cos(z), 0], [0, 0, 1]])
        return rotation_x.dot(rotation_y).dot(rotation_z).astype(np.float32)

    x, y, z = np.deg2rad(angles[0]), np.deg2rad(angles[1]), np.deg2rad(angles[2])
    rotation_x = np.array([[1, 0, 0], [0, cos(x), -sin(x)], [0, sin(x), cos(x)]])
    rotation_y = np.array([[cos(y), 0, sin(y)], [0, 1, 0], [-sin(y), 0, cos(y)]])
    rotation_z = np.array([[cos(z), -sin(z), 0], [sin(z), cos(z), 0], [0, 0, 1]])
    return rotation_z.dot(rotation_y.dot(rotation_x)).astype(np.float32)
...
###############################################################
def rotate(vertices: Any, angles: Sequence[float]) -> Any:
    """ Rotate vertices.
    X_new = R.dot(X). X: 3 x 1

    :param vertices: (n_ver, 3)
    :param angles: x, y, z angles in degree
    :return: rotated vertices, (n_ver, 3)
    """
    return vertices.dot(angle2matrix(angles).T)


def similarity_transform(
    vertices: Any, scale: float, rotation_matrix: Any, translation_3d: Sequence[float]
) -> Any:
    ...
    :param translation_3d: 3d translation vector. (3,)
    :return: transformed vertices, (n_ver, 3)
    """
    translation_3d = np.squeeze(np.array(translation_3d, dtype=np.float32))
    return scale * vertices.dot(rotation_matrix.T) + translation_3d[np.newaxis, :]
...
def to_image(vertices, h, w, is_perspective=False):
    ...
    # 为什么要把y轴反过来？ ----因为3d坐标轴中的XY轴和图片的XY轴时针顺序是反的。3d坐标轴中，x轴往右y轴往上；图片坐标轴中，x轴往右y轴往下，坐标原点在左上角。
